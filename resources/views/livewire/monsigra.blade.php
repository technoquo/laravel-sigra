<div>
    ...
    <div class="flex flex-wrap gap-5 justify-center py-8">
        @if ($this->monsigra == null)
        <div class="text-center text-gray-700 dark:text-white w-2/3">Rejoignez notre communauté dès aujourd'hui et accédez
            à un contenu exclusif ! L'abonnement est votre passe pour un monde de vidéos passionnantes. N'hésitez pas à vous
            inscrire et à faire partie de cette expérience unique !</div>
        @else
        @foreach ($this->monsigra as $video)
            <div class="flex flex-wrap gap-5 justify-center py-8">
                ...
                @empty
                    <p class="text-center text-gray-700 dark:text-white">Aucun contenu disponible pour le moment.</p>
                @endforelse
            </div>
